...
```
We initially start out with `r nrow(GM_snps)` SNPs #143259
...
rm(gm)
```
There are `r nrow(GM_snps_clean)` SNPs remaining # 119539
...
```
There are now `r nrow(geno.recode)` SNPs for `r ncol(geno.recode)` samples #(119466, 832)
...
```
A summary shows that 12 samples are flagged due to excessive no-calls
...
```
We now have genotypes for `r ncol(geno.final_merged)` samples across `r nrow(geno.final_merged)` markers #(820, 118462)
