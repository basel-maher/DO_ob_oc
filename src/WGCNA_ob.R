...
#
annot_file = read.csv("./results/flat/annot_file_ob.csv")
annot_file = annot_file[,c(2,3)]

counts = counts[which(rownames(counts) %in% annot_file$Gene.ID),]
#

#find and remove features that have fewer than 10 reads in more than 90% (190) of samples 
x=c()
for(i in 1:nrow(counts)){
  if(sum(counts[i,]<10)>=171){
    print(i)
    x = append(x,i)
  }
...
plate_batch[which(plate_batch$sample == 371),"sample"] = "371.1"
...
save(edata_ob, file = "./results/Rdata/networks/edata_ob.Rdata")

#check data
gsg = goodSamplesGenes(edata_ob, verbose = 3);
gsg$allOK

##
#cluster samples
sampleTree = hclust(dist(edata_ob), method = "average")
...
sft = pickSoftThreshold(edata_ob, powerVector = powers, verbose = 5,networkType = "signed", dataIsExpr = TRUE)
# Plot the results:
sizeGrWindow(9, 5)
par(mfrow = c(1,2));
cex1 = 0.9;
...
#make network with sft=10
net_ob = blockwiseModules(edata_ob, power = 10,
...
table(net_ob$colors)
## 10: 20 modules not including 0, 10875 genes in module 0

#saveRDS(net_ob, file="./results/Rdata/networks/wgcna_ob_10.RDS")
net_ob = readRDS("./results/Rdata/networks/wgcna_ob_10.RDS")

#get traits we want to look at
pheno = read.csv("./results/flat/full_pheno_table.csv", stringsAsFactors = FALSE)


datTraits = pheno[which(pheno$Mouse.ID %in% rownames(edata_ob)),]
datTraits = datTraits[match(rownames(edata_ob),datTraits$Mouse.ID),]
...
mergedColors = labels2colors(net_ob$colors)


# Plot the dendrogram and the module colors underneath
plotDendroAndColors(net_ob$dendrograms[[1]], mergedColors[net_ob$blockGenes[[1]]],
                    "Module colors",
...
moduleLabels = net_ob$colors
moduleColors = labels2colors(net_ob$colors)
MEs = net_ob$MEs;
geneTree = net_ob$dendrograms[[1]]
nGenes = ncol(edata_ob);
nSamples = nrow(edata_ob);

# Recalculate MEs with color labels
MEs0 = moduleEigengenes(edata_ob, moduleColors)$eigengenes
MEs = orderMEs(MEs0)
...
save(moduleTraitPvalue, file = "./results/Rdata/networks/moduleTraitPvalue_ob_full_10.RData")
save(moduleTraitCor, file = "./results/Rdata/networks/moduleTraitCor_ob_full_10.RData")
...
colnames(moduleTraitCor)[28]


#create trimmed expression object to remove grey module and get gene names from annot file
########################
combat_annot = as.data.frame(colnames(edata_ob))

combat_annot$module = net_ob$colors
combat_annot$color = moduleColors

#REMOVE GREY
rmv = combat_annot[which(combat_annot$color == "grey"),"colnames(edata_ob)"]

combat_annot = combat_annot[-which(combat_annot$`colnames(edata_ob)` %in% rmv),]
edata_ob_trim = edata_ob[,-(which(colnames(edata_ob) %in% rmv))]

combat_annot = combat_annot[,-2]

combat_annot[,c(3:4)] = annot_file[match(combat_annot$`colnames(edata_ob)`,annot_file$Gene.ID),c(1,2)]


geneModuleMembership = as.data.frame(cor(edata_ob_trim, MEs, use = "p")) # spearman or kendall? using pearson here
...
geneModuleMembership$gene = colnames(edata_ob_trim)


combat_annot[5:(ncol(geneModuleMembership)+4)] = geneModuleMembership[match(geneModuleMembership$gene,combat_annot$`colnames(edata_ob)`),]

x = match(colnames(edata_ob_trim),annot_file$Gene.ID)

colnames(edata_ob_trim) = annot_file$Gene.Name[x]


save(combat_annot, file = "./results/Rdata/networks/geneModMemAnnot_ob_p10.RData")

save(moduleTraitPvalue, file = "./results/Rdata/networks/moduleTraitPvalue_ob_full_1-.RData")

save(moduleTraitCor, file = "./results/Rdata/networks/moduleTraitCor_ob_full_10.RData")

save(edata_ob_trim, file = "./results/Rdata/networks/edata_ob_trim_10.RData")


moduleColors = moduleColors[-which(moduleColors=="grey")]
hubs = chooseTopHubInEachModule(edata_ob,moduleColors)

#Do GO analysis
##
load("./results/Rdata/networks/geneModMemAnnot_ob_p10.RData")
...
save(network_GO, file="./results/Rdata/networks/GO_ob_sft10.RData")
#####


#combine all GO results
load("./results/Rdata/networks/GO_ob_sft10.RData")
...
for (i in 1:length(network_GO)){
  ...
  colnames(networks)[col_counter] = paste0(names(network_GO)[net_counter], "_Ob_GO_term")
  
  col_counter = col_counter + 1
  
  networks[1:nrow(n),col_counter] = n$classic
  colnames(networks)[col_counter] = paste0(names(network_GO)[net_counter], "_Ob_p_value")
