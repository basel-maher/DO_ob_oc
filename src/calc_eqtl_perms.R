...
cols = readRDS("./results/Rdata/perm_cols_bone.RDS")

for(i in 1:length(cols)){
  name = colnames(cross_eqtl$pheno)[cols[i]]
  load(paste0("./results/Rdata/eqtl_perms_bone//localeqtl_bone_perm_nos_",name,".Rdata"))#loads as "perm"
...
max(perm_frame$local_autosomal)#10.83
max(perm_frame$local_X)#10.98
...
perm_frame = as.data.frame(matrix(ncol = 3, nrow=50))
cols = readRDS("./results/Rdata/perm_cols_ob.RDS")

for(i in 1:length(cols)){
  name = colnames(cross_eqtl$pheno)[cols[i]]
  load(paste0("./results/Rdata/eqtl_perms_ob/localeqtl_ob_perm_nos_",name,".Rdata"))#loads as "perm"
  autosome_lod = summary(perm, 0.05)$A[[1]] #autosomal lod threshold, alpha=0.05
  x_lod = summary(perm, 0.05)$X[[1]] #X chrom lod threshold, alpha=0.05
  perm_frame[i,1] = name
  perm_frame[i,2] = autosome_lod
...
#colnames(perm_frame) = c("gene","local_autosomal", "local_X","distal_autosomal","distal_X")
colnames(perm_frame) = c("gene","local_autosomal", "local_X")
max(perm_frame$local_autosomal)#10.88
max(perm_frame$local_X)#10.73
...
perm_frame = as.data.frame(matrix(ncol = 3, nrow=50))
cols = readRDS("./results/Rdata/perm_cols_oc.RDS")

for(i in 1:length(cols)){
  name = colnames(cross_eqtl$pheno)[cols[i]]
  load(paste0("./results/Rdata/eqtl_perms_oc/localeqtl_oc_perm_nos_",name,".Rdata"))#loads as "perm"
  autosome_lod = summary(perm, 0.05)$A[[1]] #autosomal lod threshold, alpha=0.05
  x_lod = summary(perm, 0.05)$X[[1]] #X chrom lod threshold, alpha=0.05
  perm_frame[i,1] = name
  perm_frame[i,2] = autosome_lod
...
#colnames(perm_frame) = c("gene","local_autosomal", "local_X","distal_autosomal","distal_X")
colnames(perm_frame) = c("gene","local_autosomal", "local_X")
max(perm_frame$local_autosomal)#10.84
max(perm_frame$local_X)#10.83
