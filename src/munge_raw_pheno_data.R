...
##Histo_static data
#read raw data
histo__static_data = read.csv("./data/pheno_data/histo_master_static.csv", stringsAsFactors = FALSE)

#remove "GROUP" column
histo__static_data = histo__static_data[,-2]

#remove "FV" and 'FV0" from identifier in SPEC
histo__static_data$SPEC.. = apply(histo__static_data,1,function(x) sub("FV0","",x[1]))
histo__static_data$SPEC.. = apply(histo__static_data,1,function(x) sub("FV","",x[1]))
histo__static_data$SPEC.. = apply(histo__static_data,1,function(x) sub("UVA","",x[1]))
#rename 1st col
colnames(histo__static_data)[1] = c("specimen")

#add "histo_" to columns
colnames(histo__static_data) <- paste("histo", colnames(histo__static_data), sep = "_")

##############################################
##############################################
##Histo data
#read raw data
histo_dynamic = read.csv("./data/pheno_data/histo_dynamic_master.csv", stringsAsFactors = FALSE)

#rename 1st col
colnames(histo_dynamic)[1] = c("specimen")

#add "histo_dyn_" to columns
colnames(histo_dynamic) <- paste("histo_dyn", colnames(histo_dynamic), sep = "_")

##############################################
##uCT data
#read raw data
microCT_data = read.csv("./data/pheno_data/uCT_master_all.csv",stringsAsFactors = FALSE)
...
full_pheno_table = merge(full_pheno_table,histo__static_data,by.x = "Mouse.ID",by.y = "histo_specimen",all = T,sort = FALSE)

full_pheno_table = merge(full_pheno_table,histo_dynamic,by.x = "Mouse.ID",by.y = "histo_dyn_specimen",all = T,sort = FALSE)
...
full_pheno_table = full_pheno_table[,-c(23,24,64:66)] 
...
full_pheno_table[which(is.na(full_pheno_table$MAT_VOL1)),"MAT_VOL1_nonzero"] = NA # if NA, nonzero is NA
...
full_pheno_table[which(is.na(full_pheno_table$MAT_VOL2)),"MAT_VOL2_nonzero"] = NA # if NA, nonzero is NA
...
full_pheno_table[which(is.na(full_pheno_table$MAT_VOL3)),"MAT_VOL3_nonzero"] = NA # if NA, nonzero is NA
...
full_pheno_table[which(is.na(full_pheno_table$MAT_VOL4)),"MAT_VOL4_nonzero"] = NA # if NA, nonzero is NA




#convert numerics to numeric
for(i in c(1,7:15,17:21,23:91)){
  full_pheno_table[,i] = as.numeric(full_pheno_table[,i])
}

#subset to most recently completed rows
full_pheno_table = full_pheno_table[1:804,]
